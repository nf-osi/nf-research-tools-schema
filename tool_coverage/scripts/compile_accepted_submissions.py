...
COLUMNS = {
    ...
    "vendor": [
        "vendorId", "vendorName", "vendorUrl",
    ],
    "vendorItem": [
        "vendorItemId", "vendorId", "catalogNumber", "catalogNumberURL",
        "_resourceName",
    ],
    "donor": [
        "donorId", "parentDonorId", "transplantationDonorId",
    ...
    # MutationDetails table (syn26486835) — mutation attributes
    "mutation_details": [
        "mutationDetailsId", "alleleType", "mutationType",
        "affectedGeneSymbol", "sequenceVariation", "proteinVariation",
        "_resourceName", "_pmid", "_doi", "_publicationTitle", "_year",
...
CSV_FILES = {
    ...
    "vendorItem":               "ACCEPTED_vendorItem.csv",
    "donor":                    "ACCEPTED_donor.csv",
    # Resources table feed (generated by _generate_resources_csv)
    "resources":                "ACCEPTED_resources.csv",
    "mutation_details":         "ACCEPTED_mutation_details.csv",
    "mutation":                 "ACCEPTED_mutation.csv",
}
...
def _make_resource_id(resource_name: str, ttype_plural: str) -> str:
    """Deterministic UUID5 for a Resources-table row (syn26450069).

    Uses the same namespace as other IDs in this pipeline.  The generated ID
...
_TTYPE_ID_INFO: dict = {
    "cell_line":               ("Cell Line",              "cellLineId",              "cell_lines"),
    "antibody":                ("Antibody",               "antibodyId",              "antibodies"),
    "animal_model":            ("Animal Model",           "animalModelId",           "animal_models"),
    "genetic_reagent":         ("Genetic Reagent",        "geneticReagentId",        "genetic_reagents"),
    "patient_derived_model":   ("Patient-Derived Model",  "patientDerivedModelId",   "patient_derived_models"),
...
_RESOURCE_FK_COLS = [
    ...
    "computationalToolId", "organoidProtocolId", "patientDerivedModelId", "clinicalAssessmentToolId",
]
...
def _build_mutation(d: dict) -> list[dict] | None:
    ...
    if not any([
        ...
        _get(d, "affectedGeneSymbol"), _get(d, "sequenceVariation"),
        _get(d, "proteinVariation"),
    ]):
        return None
...
def _build_computational_tool(d: dict) -> dict:
    ...
    return {
        ...
        "licenseDetails": _get(bi, "licenseDetails"),
...
def _collect_pub_and_dev_rows(json_files: list) -> tuple:
    ...
    Usage links: one row per (tool, non-development-publication) pair.
    ...
    for path in sorted(json_files):
        ...
        for pub in publications:
            # Strip "PMID:" prefix for consistent storage and lookup
            pmid = (pub.get("_pmid") or "").strip().replace("PMID:", "")
            ...
            if usage_type.lower() == "development" and resource_name:
                ...
                # Non-development publication → usage link
                usage_key = f"{pmid or doi}:{resource_name}:{ttype}"
                if usage_key not in seen_usage_keys:
                    seen_usage_keys.add(usage_key)
                    use_id = str(uuid.uuid5(
                        _PROJECT_NAMESPACE,
...
# Secondary outputs per type: list of (csv_key, builder_fn) pairs.
# builder_fn may return None to skip a row (e.g., no mutation data present).
_SECONDARY_BUILDERS: dict[str, list[tuple]] = {
    "animal_model": [
        ("mutation_details", _build_mutation_details),
...
# ---------------------------------------------------------------------------
# Derived CSV generation (vendor, vendorItem, donor)
# ---------------------------------------------------------------------------

def _generate_vendor_and_donor_csvs(csv_dir: Path, dry_run: bool) -> None:
    """
    Post-process compiled ACCEPTED_*.csv files to produce:
      ACCEPTED_vendor.csv    — unique vendors (deduped by vendorId)
      ACCEPTED_vendorItem.csv — vendor-catalog-resource triples (_resourceName for later resolution)
      ACCEPTED_donor.csv     — one donor record per cell line / PDM with demographic data
    ...
    if antibody_csv.exists():
        with open(antibody_csv, newline="", encoding="utf-8") as f:
            for row in csv.DictReader(f):
                ...
                catalog_url = (row.get("catalogURL") or "").strip()
                resource_name = (row.get("_resourceName") or "").strip()
                if not vendor_name:
                    continue
                vid = _make_vendor_id(vendor_name)
                vendors.setdefault(vid, {"vendorId": vid, "vendorName": vendor_name, "vendorUrl": ""})
                if catalog_num and resource_name:
                    vendor_items.append({
                        "vendorItemId": _make_vendor_item_id(vendor_name, catalog_num),
                        "vendorId": vid,
                        "catalogNumber": catalog_num,
                        "catalogNumberURL": catalog_url,
                        "_resourceName": resource_name,
                    })
    ...
    for csv_filename, _ in donor_sources:
        ...
        with open(type_csv, newline="", encoding="utf-8") as f:
    ...
    if dry_run:
        print(f"\n  [dry-run] vendor: {len(vendor_list)} record(s)")
        print(f"  [dry-run] vendorItem: {len(vendor_items)} record(s)")
        print(f"  [dry-run] donor: {len(donor_rows)} record(s)")
        return

    for csv_key, rows in [
        ("vendor", vendor_list),
        ("vendorItem", vendor_items),
        ("donor", donor_rows),
    ]:
        if not rows:
            continue
        out_path = csv_dir / CSV_FILES[csv_key]
        cols = COLUMNS[csv_key]
        existing_ids = set()
        id_col = cols[0]  # first column is the primary key
        if out_path.exists():
        ...
        if new_rows:
            ...
            with open(out_path, "a", newline="", encoding="utf-8") as f:
                ...
                if write_header:
                    w.writeheader()
                w.writerows({col: r.get(col, "") for col in cols} for r in new_rows)
            print(f"  {csv_key}: {len(new_rows)} new → {out_path.name}")
        else:
            print(f"  {csv_key}: 0 new (all already present)")
...
def _generate_resources_csv(
    ...
    for ttype, (rtype_label, id_col, ttype_plural) in _TTYPE_ID_INFO.items():
        ...
        if csv_key is None:
            continue
        type_csv = csv_dir / CSV_FILES.get(csv_key, "")
        if not type_csv.exists():
            continue
    ...
    if not new_rows:
        print("  resources: 0 new records (all already present)")
        return

    res_cols = [
        "resourceId", "resourceName", "resourceType",
        *_RESOURCE_FK_COLS,
        "rrid", "description", "synonyms", "usageRequirements",
        "howToAcquire", "dateAdded", "dateModified", "aiSummary",
    ]
    write_header = not resources_path.exists()
    with open(resources_path, "a", newline="", encoding="utf-8") as f:
        w = csv.DictWriter(f, fieldnames=res_cols, extrasaction="ignore")
        if write_header:
            w.writeheader()
        w.writerows(new_rows)
    print(f"  resources: {len(new_rows)} new record(s) → {resources_path.name}")
...
def compile_accepted(json_files: list, csv_dir: Path, dry_run: bool) -> None:
    ...
    for ttype, items in grouped.items():
        ...
        # Process secondary outputs (e.g., mutation_details + mutation for animal_models)
        for sec_csv_key, sec_builder_fn in _SECONDARY_BUILDERS.get(ttype, []):
            ...
            if dry_run:
                ...
                sec_count = _append_rows(sec_csv_path, sec_columns, sec_rows, dry_run=False)
                print(f"    → {sec_csv_key}: {sec_count} appended → {sec_csv_path.name}")
    ...
    usage_cols = ["usageId", "_pmid", "_doi", "_toolName", "_toolType"]
    ...
    _generate_vendor_and_donor_csvs(csv_dir, dry_run)
