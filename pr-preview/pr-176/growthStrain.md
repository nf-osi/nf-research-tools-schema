---
search:
  boost: 5.0
---

# Slot: growthStrain 


_The E. coli strain for distributing the plasmid._



<div data-search-exclude markdown="1">



URI: [nftools:growthStrain](https://w3id.org/nf-research-tools/growthStrain)
<!-- no inheritance hierarchy -->





## Applicable Classes

| Name | Description | Modifies Slot |
| --- | --- | --- |
| [GeneticReagent](GeneticReagent.md) | Genetic reagents including plasmids, viral vectors, CRISPR constructs, and ot... |  no  |






## Properties

### Type and Range

| Property | Value |
| --- | --- |
| Range | [GrowthStrainEnum](GrowthStrainEnum.md) |
| Domain Of | [GeneticReagent](GeneticReagent.md) |

### Cardinality and Requirements

| Property | Value |
| --- | --- |
### Slot Characteristics

| Property | Value |
| --- | --- |
| Owner | [GeneticReagent](GeneticReagent.md) |












## Identifier and Mapping Information





### Schema Source


* from schema: https://w3id.org/nf-research-tools




## Mappings

| Mapping Type | Mapped Value |
| ---  | ---  |
| self | nftools:growthStrain |
| native | nftools:growthStrain |




## LinkML Source

<details>
```yaml
name: growthStrain
description: The E. coli strain for distributing the plasmid.
from_schema: https://w3id.org/nf-research-tools
rank: 1000
owner: GeneticReagent
domain_of:
- GeneticReagent
range: GrowthStrainEnum

```
</details></div>